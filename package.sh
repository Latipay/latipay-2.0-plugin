...
if [ "x$LATIPAY_API" = "x" ]; then
   ...
   echo sed -i "" "s/api-staging.latipay.net/api.latipay.net/g" `grep "api-staging.latipay.net" -rl $ROOT_FOLDER | grep -v package.sh`
else
   echo "Package Plugin Connect Latipay 2.0 Test Environment"
   echo "Replace All api.latipay.net To api-staging.latipay.net"
   sed -i "" "s/api.latipay.net/api-staging.latipay.net/g" `grep "api.latipay.net" -rl $ROOT_FOLDER | grep -v package.sh`
fi
