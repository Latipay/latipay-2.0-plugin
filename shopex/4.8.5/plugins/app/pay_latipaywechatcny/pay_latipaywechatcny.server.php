...
class pay_latipaywechatcny extends paymentPlugin{
    function pay_latipaywechatcny_callback($in,&$paymentId,&$money,&$message,&$tradeno){
		$json = file_get_contents('php://input');
		error_log(date('Y-m-d H:i:s',time())."\tcallback.server.json=".$json."\n",3,HOME_DIR."/logs/latipaywechatcny-".date('Y-m-d',time()).".log");
